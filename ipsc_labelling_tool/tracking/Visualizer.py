...
class VisualizerParams:
    ...
    :ivar mode: '0: annotations, 1: detections, 2: tracked',
    ...
    :ivar convert_to_rgb: 'convert the image to RGB before showing it; this is sometimes needed if the raw frame is'
              ...
              'interchanged)',
    :ivar write_frame_id: 'Write the current free mighty in the frames',
    ...
    def __init__(self):
        ...
        """
        self.mode = 0
        self.cols = ('green', 'red', 'blue', 'cyan', 'magenta', 'purple', 'orange', 'forest_green')
...
class Visualizer:
    ...
    def initialize(self, save_fname_templ, frame_size, pause_after_frame=None):
        ...
        if self._params.out_size:
            self.out_size = self._params.out_size
        elif self._params.resize_factor != 1:
            self.out_size = (int(frame_size[0] * self._params.resize_factor),
        ...
        if self._params.save_fmt[0] in self.image_exts:
            ...
            if cv2.__version__.startswith('3'):
                writer.open(filename=save_path, apiPreference=cv2.CAP_FFMPEG,
                            ...
                            fps=int(self._params.save_fmt[2]), frameSize=self.out_size)
            else:
                writer.open(filename=save_path, fourcc=cv2.cv.CV_FOURCC(*self._params.save_fmt[1]),
                            fps=self._params.save_fmt[2], frameSize=self.out_size)
            if not writer.isOpened():
                self._logger.error('Video file {:s} could not be opened'.format(save_path))
                return False

            self._logger.info('Saving {} x {} video to {:s}'.format(self.out_size[0], self.out_size[1], save_path))

        self.writer = writer
        return True

    def update(self, frame_id, frame, frame_data, masks=(), label='', crop_size=(), filename=None):
        """
        :type frame_id: int
        ...
        for ann_id in range(ann_data.shape[0]):
            ...
            if target_id < 0:
                ...
                col_id = (target_id - 1) % len(self._params.cols)
                col = self._params.cols[col_id]
            ...
            if self._params.show_trajectory:
                ...
                utils.drawTrajectory(ann_frame, self.traj_data[2][target_id], color=col,
            ...
            if self._params.show_label:
                label = ann_data[ann_id, 6]
                kw_args['label'] = label
            if self._params.show_id:
                kw_args['_id'] = target_id
            if masks:
        ...
        ann_frame = utils.resizeAR(ann_frame, self.out_size[0], self.out_size[1])

        if img_text and self._params.show_header:
