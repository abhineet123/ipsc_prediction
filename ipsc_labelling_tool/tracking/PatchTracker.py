...
class PatchTracker:
    def __init__(self, params, logger, target_id, label='generic', show_only=False):
        ...
        if not self.show_only:
            tracker_type = str(self._params.tracker_type)
            if tracker_type in ('0', 'cv'):
                self.tracker_type = 0
                ...
                print('Using OpenCV {:s} tracker'.format(tracker_type))
                if int(minor_ver) < 3:
            ...
            elif tracker_type in ('4', 'da_siam_rpn'):
                self.tracker_type = 4
                ...
                raise IOError('Invalid tracker_type: {}'.format(tracker_type))
