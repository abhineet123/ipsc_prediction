...
def drawBox(frame, box, _id=None, color='black', thickness=2, label=None, mask=()):
    ...
    box = [int(x) for x in box.squeeze()]
    ...
    if box[1] > 10:
        ...
        y_loc = int(box[3] - 1)
    if box_str:
        cv2.putText(frame, box_str, (int(box[0] - 1), y_loc), cv2.FONT_HERSHEY_COMPLEX_SMALL,
    ...
    if mask:
        ...
        mask_cv = mask.reshape((-1, 1, 2))
        cv2.drawContours(frame, mask_cv, -1, color, thickness=thickness)

        h, w = frame.shape[:2]
        ann_mask = np.zeros((h, w), dtype=np.uint8)

        contour_pts = np.array(mask).reshape((-1, 2))
        contour_pts2 = np.asarray([contour_pts, ], dtype=np.int32)
        # contour_pts2 = mask_cv.reshape((1, -1, 2))

        ann_mask = cv2.fillPoly(ann_mask, contour_pts2, 255)

        # cv2.imshow('ann_mask', ann_mask)

        ann_mask_binary = ann_mask > 0

        ann_mask_img = np.zeros_like(frame)
        ann_mask_img[ann_mask_binary] = color

        frame[ann_mask_binary] = (alpha * frame[ann_mask_binary] +
                                    (1 - alpha) * ann_mask_img[ann_mask_binary])
