...
def get_max_overlap_obj(objects, location):
    ...
    if objects.shape[0] == 0:
        ...
        """single object"""
        iou = np.empty((1, 1))
        compute_overlap(iou, None, None, objects[0, 2:6].reshape((1, 4)), location)
        max_iou_idx = 0
        max_iou = iou
    else:
        """get object with maximum overlap with the location"""
        iou = np.empty((objects.shape[0], 1))
        compute_overlaps_multi(iou, None, None, objects[:, 2:6], location)
...
def resize_ar(src_img, width=0, height=0, return_factors=False,
              placement_type=1, only_border=0, only_shrink=0, resize_factor=None, size=None, auto_max=1):
    if size is not None:
        width, height = size
    ...
    if auto_max and width > 0 and height > 0:
        ...
        width, height = get_size(width, height, src_height, src_width, src_aspect_ratio, only_shrink, only_border)
...
def draw_boxes(frame, boxes, ids=None, colors='black', thickness=2,
               is_dotted=0, transparency=0., masks=None, text_cols=None, labels=None, **kwargs):
    ...
    if not isinstance(text_cols, (list, tuple)):
        text_cols = [text_cols, ] * n_boxes

    if not isinstance(colors, (list, tuple)):
        colors = [colors, ] * n_boxes

    if text_cols[0] is not None and masks is not None:
        """text and bbox in same color and mask_rgb in the main box_col"""
        box_cols = text_cols
    else:
        box_cols = colors

    if not isinstance(is_dotted, (list, tuple)):
        is_dotted = [is_dotted, ] * n_boxes

    if not isinstance(transparency, (list, tuple)):
        transparency = [transparency, ] * n_boxes

    assert len(ids) == n_boxes, "mismatch between number of boxes and ids"
    assert len(box_cols) == n_boxes, "mismatch between number of boxes and box_cols"
    assert len(is_dotted) == n_boxes, "mismatch between number of boxes and is_dotted"
    assert len(transparency) == n_boxes, "mismatch between number of boxes and transparency"
    assert len(labels) == n_boxes, "mismatch between number of boxes and labels"
    assert len(text_cols) == n_boxes, "mismatch between number of boxes and text_cols"

    for box, _id, box_col, _is_dotted, _transparency, label, text_col in zip(boxes, ids, box_cols, is_dotted,
                                                                             transparency, labels, text_cols):
        draw_box(frame, box,
                 _id=_id,
                 color=box_col,
                 thickness=thickness,
    ...
    if masks is not None:
        assert len(masks) == n_boxes, "number of mask_rgb points does not match the number of boxes"

        mask_rgb = np.zeros_like(frame, dtype=np.uint8)
        for box_col, _mask_pts in zip(colors, masks):
            mask_rgb = cv2.fillPoly(mask_rgb, np.array([_mask_pts, ], dtype=np.int32), col_bgr[box_col])
        mask_gs = cv2.cvtColor(mask_rgb, cv2.COLOR_BGR2GRAY)
        mask_binary = mask_gs > 0
        frame[mask_binary] = (0.5 * frame[mask_binary] + 0.5 * mask_rgb[mask_binary]).astype(
            np.uint8)
...
def draw_box(frame, box, _id=None, color='black', thickness=2,
             is_dotted=0, transparency=0.,
             text_col=None,
             font=cv2.FONT_HERSHEY_TRIPLEX, font_size=0.5, label=None):
    """
    ...
    if label is not None:
        ...
        text_x, text_y = int(box[0] - 1), int(box[1] - 10)
        if text_y - 25 <= 0:
            """obj too close to the top edge"""
            text_y = int(box[1] + box[3] + 25)

        if text_x <= 0:
            """obj too close to the left edge"""
            text_x = int(box[0] + box[2] + 1)

        cv2.putText(frame, label, (text_x, text_y),
                    # cv2.FONT_HERSHEY_SIMPLEX,
                    font, font_size, text_col, 1, font_line_type)
...
class CVText:
    def __init__(self, color='white', bkg_color='black', location=0, font=5,
                 size=0.8, thickness=1, line_type=2, offset=(5, 25), fmt=None):
        """

        :param color:
        :param bkg_color: 'should be empty for no background',
        :param location: '0: top left, 1: top right, 2: bottom right, 3: bottom left; '
        :param font: 'Available fonts: '
                    '0: cv2.FONT_HERSHEY_SIMPLEX, '
                    '1: cv2.FONT_HERSHEY_PLAIN, '
                    '2: cv2.FONT_HERSHEY_DUPLEX, '
                    '3: cv2.FONT_HERSHEY_COMPLEX, '
        ...
        if fmt is not None:
            ...
            self.bkg_color = fmt.bkg_color
            self.location = fmt.location
            self.font = fmt.font
            self.size = fmt.size
            self.thickness = fmt.thickness
            self.line_type = fmt.line_type
...
def annotate_and_show(title, img_list, text=None, pause=1,
                      fmt=None, no_resize=1, grid_size=(-1, 1), n_modules=3,
...
class SCP:
    ...
    def __init__(self):
        ...
        self.code_path = 'deep_mdp/deep_mdp'
        self.auth_data = {}
