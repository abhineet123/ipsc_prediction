...
class Canvas(QWidget):
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        if self.canCloseShape() and len(self.current) > 3:
            ...
            curr_shape = self.selectedShape
            is_dummy = 0
            if curr_shape is None:
                frames_reader = self.main_window.frames_reader
                img_bbox = [0, 0, frames_reader.width - 1, frames_reader.height - 1]
                curr_shape = Shape(self.main_window.default_label, bbox=img_bbox)
                is_dummy = 1

            masks = curr_shape.addMask(self.main_window.image_np,
            ...
            if n_new_objs:
                print('adding {} new objects'.format(n_new_objs))

            for mask in masks:
                self.add_shape(mask, bbox=None,
    ...
    def add_shape(self, mask, bbox, label, bbox_source, id_number):
        is_gate = label == "gate"
        shape = Shape(label, mask=mask, bbox_source=bbox_source, id_number=id_number, is_gate=is_gate)

        if mask is not None and mask:
            if len(mask[0]) == 2:
                mask = [(x, y, 1) for x, y in mask]
            mask_arr = np.asarray(mask)
            xmin, ymin, _ = np.min(mask_arr, axis=0)
            xmax, ymax, _ = np.max(mask_arr, axis=0)
        else:
            assert bbox is not None, "both bbox and mask cannot be None"
