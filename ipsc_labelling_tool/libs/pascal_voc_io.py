...
class PascalVocWriter:
    ...
    def __init__(self, foldername, filename, imgSize, databaseSrc='Unknown', localImgPath=None, save_mask_img=0,
        ...
        self.verified = False
        self.save_mask_img = save_mask_img
    ...
    def addBndBox(self, xmin, ymin, xmax, ymax, name, difficult, bbox_source, id_number, score,
                  mask, mask_img=None, parents=None, children=None):
        bndbox = {
            'xmin': xmin,
            'ymin': ymin,
            'xmax': xmax,
    ...
    def appendObjects(self, top):
        ...
        for obj_id, each_object in enumerate(self.boxlist):
            ...
            if self.enable_hierarchy:
                parents = each_object['parents']
                children = each_object['children']

                if parents is not None and parents:
                    parents_txt = ','.join(str(parent) for parent in parents)
                    parents_elem = SubElement(object_item, 'parents')
                    parents_elem.text = parents_txt

                if children is not None and children:
                    children_txt = ','.join(str(child) for child in children)
                    children_elem = SubElement(object_item, 'children')
                    children_elem.text = children_txt

            if mask_img is not None:
                if self.save_mask_img:
                ...
                if mask_pts is None or not mask_pts:
                    ...
                    if mask_img_bin is not None:
                        mask_pts = Shape.contourPtsFromMask(mask_img_bin.astype(np.uint8), allow_multi=0)
    ...
    def save(self, targetFile=None, _filename=None, _imgSize=None):
        ...
        with portalocker.Lock(self.out_fname, 'w', timeout=100) as out_file:
...
class PascalVocReader:
    ...
    def __init__(self, filepath, enable_hierarchy=False):
        # shapes type:
    ...
    def addShape(self, label, bndbox, difficult, bbox_source, id_number, score, mask,
                 mask_img=None, parents=None, children=None):
        ...
        shape = [label, points, None, None, difficult, bbox_source, id_number, score, mask, mask_img]
        if self.enable_hierarchy:
            shape.append(parents)
            shape.append(children)
    ...
    def parseXML(self):
        ...
        for object_iter in xmltree.findall('object'):
            ...
            try:
                parents = object_iter.find('parents').text
            except AttributeError:
                parents = None
            else:
                parents = [int(k) for k in parents.split(',') if k]

            try:
                children = object_iter.find('children').text
            except AttributeError:
                children = None
            else:
                children = [int(k) for k in children.split(',') if k]
            ...
            self.addShape(label, bndbox, difficult, bbox_source, id_number, score, mask, mask_img, parents, children)
