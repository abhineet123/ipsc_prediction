...
import threading
import socket
from libs.netio import send_msg_to_connection
from tracking.Visualizer import Visualizer, VisualizerParams
from tracking.Utilities import linux_path
from tracking import Server
...
from libs.frames_readers import VideoReader, DirectoryReader, get_frames_reader
...
from to_mask_seq import saveMasks
...
def save_roi_local(frames_reader, dirname, roi_save_dir, show_roi):
    if isinstance(frames_reader, DirectoryReader):
        dir_path = dirname
    ...
    elif isinstance(frames_reader, VideoReader):
        video_path = frames_reader.video_fn
    ...
    for i in tqdm(range(n_frames)):
        # fname = 'frame{:06d}.jpg'.format(i + 1)
        roi_img = frames_reader.get_frame(i)
        file_path = frames_reader.get_file_path()
        file_name = os.path.basename(file_path)
        out_fname = os.path.join(roi_save_path, file_name)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, params, logger):
        ...
        default_filename = self.params.file_name
        if params.root_dir:
        ...
        self.loadPredefinedClasses(default_prefdef_class_file)
        ...
        self.ask_for_target_id = 0

        # Create a widget for using default label
        self.target_id_checkbox = QCheckBox(u'Ask for target ID')
        self.target_id_checkbox.setChecked(False)
        self.target_id_checkbox.stateChanged.connect(self.default_label_checkbox)

        self.use_default_label_checkbox = QCheckBox(u'Use default label')
        self.use_default_label_checkbox.setChecked(True)
        self.use_default_label_checkbox.stateChanged.connect(self.default_label_checkbox)

        self.default_label = self.labelHist[0]

        self.default_label_combo_box = QComboBox()
        self.default_label_combo_box.setEditable(False)
        self.default_label_combo_box.addItems(self.labelHist)

        self.default_label_combo_box.activated.connect(self.set_default_label)
        self.default_label_combo_box.currentIndexChanged.connect(self.set_default_label)
        self.default_label_combo_box.editTextChanged.connect(self.set_default_label)

        self.default_label_combo_box.setCurrentText(self.default_label)

        # self.defaultLabelTextLine = QLineEdit()
        # self.defaultLabelTextLine.setText('bear')

        use_default_label_qh_box_layout = QHBoxLayout()
        use_default_label_qh_box_layout.addWidget(self.use_default_label_checkbox)
        use_default_label_qh_box_layout.addWidget(self.default_label_combo_box)
        use_default_label_container = QWidget()
        use_default_label_container.setLayout(use_default_label_qh_box_layout)

        target_id_checkbox_layout = QHBoxLayout()
        target_id_checkbox_layout.addWidget(self.target_id_checkbox)
        target_id_checkbox_container = QWidget()
        target_id_checkbox_container.setLayout(target_id_checkbox_layout)
        ...
        listLayout.addWidget(self.edit_button)
        # listLayout.addWidget(self.diffcButton)
        listLayout.addWidget(use_default_label_container)
        listLayout.addWidget(target_id_checkbox_container)
        ...
        track_all = action('Track\nAll Objects', self.trackAllObjects,
                           ['Shift+Space'], 'track_all', u'Track all objects (Shift+Space)', enabled=True)
        ...
        self.delete_bin = settings.get(SETTING_DELETE_BIN)
        if self.delete_bin is None:
            self.delete_bin = self.params.delete_bin

        toggle_delete_bin = action("Delete\nbinary files", self.toggle_delete_bin, checkable=True)
        toggle_delete_bin.setChecked(self.delete_bin)
        ...
        self.actions = struct(save=save, saveAs=saveAs, open=openVideo, close=close,
                              ...
                              beginner=(), advanced=(),
                              editMenu=(
                                  edit, create_mask, track_all, copy, delete, delete_target, delete_all_in_curr_image,
                                  delete_all_annotations,
                                  track, set_roi),
                              beginnerContext=(
                                  create, create_mask, track_all, edit, copy, delete, delete_target, track, set_roi),
                              advancedContext=(createMode, create_mask, track_all, editMode, edit, copy,
        ...
        self.actions.beginner = (
            openVideo, opendir, openNextImg, openPrevImg, save, None, create, create_mask, track_all,
        ...
        if load_prev:
            ...
            if prev_dir is not None:
                if os.path.isdir(prev_dir):
                    ...
                    print('Previous directory does not exist: {}'.format(prev_dir))
                    prev_file = self.settings.get(SETTING_LAST_OPEN_VIDEO_FILE)
                    if prev_file and prev_file is not None:
                        if os.path.isfile(prev_file):
                            print('Loading previous file: {}'.format(prev_file))
                            self.queueEvent(partial(self.openVideoFile, prev_file))
            ...
            if os.path.isdir(self.filePath):
                self.queueEvent(partial(self.openDir, self.filePath))
            elif os.path.isfile(self.filePath):
                self.queueEvent(partial(self.openVideoFile, self.filePath))
            else:
                self.queueEvent(partial(self.openDir, ""))
    ...
    def loadLabels(self, shapes):
        s = []
        for label, points, line_color, fill_color, difficult, bbox_source, id_number, score, \
    ...
    def saveLabels(self, annotationFilePath):
        ...
        try:
            if self.usingPascalVocFormat is True:
                ...
                file_path = self.frames_reader.file_path
                self.labelFile.savePascalVocFormat(annotationFilePath, shapes, file_path, self.imageData,
    ...
    def newShape(self):
        ...
        if not self.use_default_label_checkbox.isChecked() or not self.default_label_combo_box.currentText():
            ...
            text = self.default_label_combo_box.currentText()
    ...
    def loadFile(self, filePath=None):
        ...
        # print('loadFile: filePath: {}'.format(filePath))
        ...
        seq_title = self.full_title
        if self.roi is not None:
            seq_title = '{} ({}_{}_{}_{})'.format(seq_title,
                                                  self.roi['xmin'], self.roi['ymin'],
                                                  self.roi['xmax'], self.roi['ymax'])
        self.setWindowTitle('{} ({})'.format(seq_title, filePath))
    ...
    def closeEvent(self, event):
        ...
        if not all_windows_closed:  # or not self.mayContinue():
            ...
            settings[SETTING_DELETE_BIN] = self.delete_bin
    ...
    def load_masks(self):
        ...
        for file in files:
            ...
            for _id in range(n_shapes):
                ...
                contour_pts, mask_pts = Shape.contourPtsFromMask(new_mask_img)
    ...
    def openDir(self, dirpath=None):
        ...
        if dirpath:
            ...
            frames_reader = DirectoryReader(dirpath, save_as_bin=self.params.save_as_bin)
            ...
            seq_title = os.path.basename(dirpath)

            self.full_title = '{} ({})'.format(self.base_title, seq_title)
    ...
    def edge_detection(self):
        ...
        if hed_mask is not None:
            _, mask_pts = curr_shape.contourPtsFromMask(hed_mask)
    ...
    def generate_mask(self):
        ...
        if os.path.isfile(xmlPath):
            tVocParseReader = PascalVocReader(xmlPath, enable_hierarchy=True)
    ...
    def saveSeqSettings(self):
        ...
        try:
            ...
            print(f'saving of sequence settings failed: {e}')

    def openVideoFile(self, filename=None):
        if not self.mayContinue():
            print('may not Continue')
            return
        ...
        frames_reader = VideoReader(filename, save_as_bin=self.params.save_as_bin)
        if frames_reader.num_frames == 0:
            QMessageBox.critical(self, "Error", "No frames detected! Please choose different path.", QMessageBox.Ok)
            del frames_reader
            return
        ...
        first_img = self.mImgList[0]

        base_filename = os.path.basename(filename)
        self.full_title = '{} ({})'.format(self.base_title, base_filename)

        # print('first_img: {}'.format(first_img))
        # print('filename: {}'.format(filename))
        # print('base_filename: {}'.format(base_filename))
        # print('full_title: {}'.format(self.full_title))

        self.loadFile(first_img)
        self.loadSeqSettings()
    ...
    def createMask(self):
        ...
        if curr_shape is None:
            img_bbox = [0, 0, self.frames_reader.width - 1, self.frames_reader.height - 1]
            curr_shape = Shape(self.default_label, bbox=img_bbox)

            is_dummy = 1

        masks = curr_shape.addMask(self.image_np,
                                   self.params.mask,
                                   self.augment,
                                   self.hed_net
                                   )
        n_all_objs = len(masks)
        print('is_dummy: {}'.format(is_dummy))
        print('n_all_objs: {}'.format(n_all_objs))

        if not is_dummy:
            """already added first mask to the current object"""
            masks = masks[1:]

        n_new_objs = len(masks)
        if n_new_objs:
            print('adding {} masks'.format(n_new_objs))

            for mask in masks:
                self.canvas.add_shape(mask, bbox=None,
                                      label=curr_shape.label,
                                      bbox_source=curr_shape.bbox_source,
                                      id_number=None)
    ...
    def saveROI(self):
        ...
        import copy
        frames_reader = copy.deepcopy(self.frames_reader)

        from threading import Thread
        background_thread = Thread(target=save_roi_local, args=(frames_reader, self.dirname, roi_save_dir,
                                                                self.params.show_roi))
        ...
        if copy_existing_annotations:
            ...
            for file in files:
                if os.path.isfile(file):
                    reader = PascalVocReader(file, enable_hierarchy=True)
    ...
    def trackAllObjects(self):
        if not self.canvas.shapes:
            print('no objects to track')
            return

        curr_index = self.mImgList.index(self.filePath)
        n_objects = len(self.canvas.shapes)

        print('tracking {} objects'.format(n_objects))
        n_parallel_jobs = 1

        if n_objects > self.params.max_parallel_track > 0:
            print('tracking {} objects in parallel'.format(self.params.max_parallel_track))
            n_parallel_jobs = int(np.ceil(float(n_objects) / self.params.max_parallel_track))

        # print('n_parallel_jobs: {}'.format(n_parallel_jobs))

        for job_id in range(n_parallel_jobs):
            start_shape_id = job_id * self.params.max_parallel_track
            end_shape_id = min(start_shape_id + self.params.max_parallel_track, n_objects)

            if n_parallel_jobs > 1:
                print('running parallel tracking job {} / {} for objects {} to {}'.format(
                    job_id + 1, n_parallel_jobs, start_shape_id + 1, end_shape_id))

            track_procs = []

            for shape_id in range(start_shape_id, end_shape_id):
                curr_shape = self.canvas.shapes[shape_id]
                try:
                ...
                if curr_shape.id_number is not None and curr_shape.id_number >= 0:
                    ...
                    track_id = int(curr_shape.id_number)
                else:
                    track_id = self.max_id + 1
                    self.max_id += 1

                track_proc = self.send_patch_tracking_request(
                    curr_index, bbox, curr_shape.label, track_id, 'ground_truth')

                if track_proc is not None:
                    track_procs.append(track_proc)

            if n_parallel_jobs > 1:
                for p in track_procs:
                    p.wait()
    ...
    def send_patch_tracking_request(self, frame_number, bbox, label,
        ...
        if self.params.tracking_mode == 2:

            _logger.info('Running in threaded function call mode')
            ...
            if self.server_params is None:
                ...
                tracking_dir = linux_path(script_dir, 'tracking')

                self.server_params = Server.Server.Params()

                """change working directory to be the same as the tracking server before parsing the 
                corresponding CFG files so that any paths specified therein work"""
                # print('changing working directory to {}'.format(tracking_dir))
                os.chdir(tracking_dir)
                paramparse.process(self.server_params)
                # print('changing working directory to {}'.format(script_dir))
                os.chdir(script_dir)

                self.server_params.mode = 0
                # self.server = Server.Server(server_params, self.logger)
            ...
            src_path = self.frames_reader.get_path().replace(os.sep, "/")
            ...
            frames_reader = get_frames_reader(src_path, save_as_bin=self.server_params.save_as_bin)
            if self.roi is not None:
            ...
            if self.params.enable_logging:
                ...
                log_dir = linux_path('tracking', 'log')
                out_fname = time_stamp + '.ansi'
                zip_fname = out_fname.replace('.ansi', '.zip')

                out_path = linux_path(log_dir, out_fname)
                zip_path = linux_path(log_dir, zip_fname)
    ...
    def augment(self, request, mask=None, use_prev=0, save_seq=1):
        ...
        if isinstance(self.frames_reader, VideoReader):
            print('augmentation is only supported for image sequence data')
            return
        ...
        objects_to_pts, _ = Shape.getContourPts(mask, verbose=0)

        # for obj_id in range(len(objects_to_pts)):
        #     objects_to_pts[obj_id] = [[(x - xmin), (y - ymin)] for x, y in objects_to_pts[obj_id]]
        ...
        # col = 1
        col = 255
        mask_img, _ = Shape.contourPtsToMask(objects_to_pts, patch_img, col=(col, col, col))
    ...
    def visualize(self, request):
        ...
        for frame_id in range(init_frame_id, n_frames):
            ...
            xml_reader = PascalVocReader(xml_path, enable_hierarchy=True)
            shapes = xml_reader.getShapes()
            frame_data = []
            masks = []

            for shape in shapes:
                label, points, _, _, difficult, bbox_source, id_number, score, mask, _, _, _ = shape
                ...
                frame_data.append([frame_id, target_id, xmin, ymin, width, height, label])
                if mask is not None:
                    objects_to_pts, _ = Shape.getContourPts(mask, verbose=0)
                    _contour_pts = []
                    if objects_to_pts:
                        _contour_pts = objects_to_pts[0]
                    masks.append(_contour_pts)

            frame_data = np.asarray(frame_data)
            if not visualizer.update(frame_id, curr_frame, frame_data, masks, filename=filename_noext):
                break
    ...
    def target_id_checkbox(self):
        self.ask_for_target_id = self.target_id_checkbox.isChecked()

    def default_label_checkbox(self):
        self.default_label_combo_box.setEnabled(self.use_default_label_checkbox.isChecked())

    def set_default_label(self):
        self.default_label = self.default_label_combo_box.currentText()
    ...
    def go_to_button_clicked(self):
        ...
        print('curr_index: {}'.format(self.curr_index))
        # except BaseException as e:
        #     print('go_to_button_clicked failed: {}'.format(e))
    ...
    def save_masks(self):
        ...
        saveMasks(os.path.dirname(self.saveDir), self.saveDir, self.out_mask_size,
    ...
    class Params:
        ...
        :type mask: MainWindow.Params.Mask
        :type visualizer: VisualizerParams

        :ivar tracking_mode: 0: tracking server
                            1: parallel execution through python processes
                            2: parallel execution through multithreaded function calls
        """
        ...
        def __init__(self):
            ...
            self.help = {
                ...
                'max_boxes': 'max_boxes to retain per image - can be used to remove nonexistent annotations from xml '
                             'files',
