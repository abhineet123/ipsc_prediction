...
class MojowRocksBase(CustomDataset):
    def load_annotations(self, ann_file):
        ...
        for _class_id, _class_name in enumerate(extra_classes):
            cat_id = _class_id + max_coco_class_id + 1
            cat_dict = {
        ...
        if len(self.coco.imgs) == 0:
            ...
            ann_file_dir = os.path.dirname(ann_file)
            ann_file_name = os.path.basename(ann_file)
            image_dir_name = os.path.splitext(ann_file_name)[0]
            image_dir_path = os.path.join(ann_file_dir, image_dir_name).replace(os.sep, '/')

            assert os.path.isdir(image_dir_path), f"image_dir_path does not exist: {image_dir_path}"

            src_file_gen = [[os.path.join(dirpath, f).replace(os.sep, '/') for f in filenames if
                             os.path.splitext(f.lower())[1] in img_exts]
                            for (dirpath, dirnames, filenames) in os.walk(
                    image_dir_path, followlinks=True)]
            img_paths = [item for sublist in src_file_gen for item in sublist]

            n_images = len(img_paths)
            assert n_images > 0, f"no images found in: {image_dir_path}"

            print(f'found {n_images} images in {image_dir_path}')

            self.coco.dataset['images'] = []

            for img_path in img_paths:
                image_rel_path = os.path.relpath(img_path, ann_file_dir).rstrip('.' + os.sep).replace(os.sep, '/')
                img_id = os.path.splitext(image_rel_path)[0]
                ...
                self.coco.dataset['images'].append(img_dict)
                self.coco.imgs[img_id] = img_dict.copy()
